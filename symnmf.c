...
/* Checks convergence by Frobenius Norm */
int check_convergence(double** A, double** B,int n,int k){
    double sum, curr;
    int i, j;
    sum = 0.0;
...
/* reads matrix from file and writes it into datapoints */
void get_datapoints(FILE* file, double** datapoints, int n, int d) {
    int i,j;
    int check;
    for (i = 0; i < n; i++) {
...
/* Updates H to solve minH≥0||W − HHt|| */
void update_H(double** H, int n, int k, double** W){
    double **wh, **hhh, **hh;
    int i, j;
...
/* Main function for the program, gets goal and data 
