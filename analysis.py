...
def main():
    ...
    file_in = sys.argv[2]
    file = open(file_in)
    import_points(file) ## Reads file and saves the data in all_vectors
    str_k = sys.argv[1]
    ...
    if not is_valid(k, default_iter):
        return
    init(k) ## Initialize clusters 
    kMean(k) ## Sets centers to be the clusters when implementing K-Means algorithm
    h = symnmf.final_H(all_vectors, len(all_vectors), len(all_vectors[0]), k) ## Sets h to be the clusters when implementing Symnmf algorithm
